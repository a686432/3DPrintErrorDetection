...
glmDraw(GLMmodel* model, GLuint mode)
{
  ...
  while (group) {
	...
	glColor3f(0.0f, 1.0f, 0.0f); // 颜色设置为蓝色
	...
	//	GLfloat* v = new GLfloat[3];
		...
		//	v[0] = -(&model->facetnorms[3 * T(group->triangles[i]).findex])[0];
		//	v[1] = -(&model->facetnorms[3 * T(group->triangles[i]).findex])[1];
		//	v[2] = -(&model->facetnorms[3 * T(group->triangles[i]).findex])[2];
